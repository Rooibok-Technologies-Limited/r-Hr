...
class Warning extends BaseController {
	...
	public function update_warning() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$WarningModel = new WarningModel();
				$result = $WarningModel->where('company_id', $this->tenantCompanyId())->update($id,$data);	
				$Return['csrf_hash'] = csrf_hash();	
				if ($result == TRUE) {
	...
	public function delete_warning() {
		...
		if($this->request->getPost('type')=='delete_record') {
			...
			$Return['csrf_hash'] = csrf_hash();
			$WarningModel = new WarningModel();
			$result = $WarningModel->where('warning_id', $id)->where('company_id', $this->tenantCompanyId())->delete($id);
			if ($result == TRUE) {
