...
class Transfers extends BaseController {
	...
	public function update_transfer() {
		...
		if (strip_tags(trim($this->request->getPost('type'))) === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$TransfersModel = new TransfersModel();
				$result = $TransfersModel->where('company_id', $this->tenantCompanyId())->update($id,$data);	
				$Return['csrf_hash'] = csrf_hash();	
				if ($result == TRUE) {
					if($status == 1){
						// employee details
	...
	public function delete_transfer() {
		...
		if($this->request->getPost('type')=='delete_record') {
			...
			$Return['csrf_hash'] = csrf_hash();
			$TransfersModel = new TransfersModel();
			$result = $TransfersModel->where('transfer_id', $id)->where('company_id', $this->tenantCompanyId())->delete($id);
			if ($result == TRUE) {
