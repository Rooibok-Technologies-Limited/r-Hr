...
class Holidays extends BaseController {
	...
	public function update_holiday() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$HolidaysModel = new HolidaysModel();
				$result = $HolidaysModel->where('company_id', $this->tenantCompanyId())->update($id, $data);	
				$Return['csrf_hash'] = csrf_hash();	
				if ($result == TRUE) {
	...
	public function delete_holiday() {
		...
		if($this->request->getPost('type')=='delete_record') {
			...
			$Return['csrf_hash'] = csrf_hash();
			$HolidaysModel = new HolidaysModel();
			$result = $HolidaysModel->where('holiday_id', $id)->where('company_id', $this->tenantCompanyId())->delete($id);
			if ($result == TRUE) {
