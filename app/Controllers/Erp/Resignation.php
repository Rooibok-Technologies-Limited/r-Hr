...
class Resignation extends BaseController {
	...
	public function update_resignation() {
		...
		if (strip_tags(trim($this->request->getPost('type'))) === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$ResignationsModel = new ResignationsModel();
				$result = $ResignationsModel->where('company_id', $this->tenantCompanyId())->update($id,$data);	
				$Return['csrf_hash'] = csrf_hash();	
				if ($result == TRUE) {
	...
	public function delete_resignation() {
		...
		if($this->request->getPost('type')=='delete_record') {
			...
			$Return['csrf_hash'] = csrf_hash();
			$ResignationsModel = new ResignationsModel();
			$result = $ResignationsModel->where('resignation_id', $id)->where('company_id', $this->tenantCompanyId())->delete($id);
			if ($result == TRUE) {
