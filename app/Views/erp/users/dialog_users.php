...
$user_id = !empty($field_id) ? udecode($field_id) : false;
$result = $user_id ? $UsersModel->where('user_id', $user_id)->first() : null;
if(empty($result)) {
	echo '<div class="modal-header"><h5 class="modal-title">User</h5><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button></div><div class="modal-body"><p class="text-muted mb-0">User not found.</p></div>';
	return;
}
?>
<div class="modal-header">
  <h5 class="modal-title"><?= esc($result['first_name'].' '.$result['last_name']); ?></h5>
